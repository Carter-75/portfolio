...
export default function Navbar() {
  ...
  return (
    <nav
      ...
      style={scrolled ? { boxShadow: '0 2px 32px rgba(0,0,0,0.5)' } : {}}
    >
      <div className="container">
        <div className="navbar-brand">
          ...
          >
            <figure
              className="image is-32x32 navbar-brand-avatar"
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                triggerEasterEgg();
          ...
          <div className="navbar-end">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`navbar-item nav-link-item ${isActive(link.href) ? 'is-active' : ''}`}
                role="menuitem"
